  ...
  const addToCart = (product) => {
    ...
    if (existing) {
      if (existing.quantity >= product.quantity) {
        setError(`Cannot exceed available stock of ${product.quantity} for ${product.name}`);
        return;
      }
      setCart(
        cart.map((item) =>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 bg-white rounded-lg border border-slate-200 shadow-sm">
              ...
              <p className="text-sm text-slate-500 font-medium">Fetching product catalog...</p>
            </div>
          ) : (
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm space-y-6">
            ...
            <div>
              ...
              {showAddCustomer ? (
                <form onSubmit={handleAddCustomerSubmit} className="space-y-2 border border-slate-200 rounded p-3 bg-slate-50">
                  <input
            ...
            {/* Cart Items */}
            <div>
              <h3 className="text-sm font-semibold text-slate-700 mb-2">Cart Items</h3>
              <div className="divide-y divide-slate-100 max-h-64 overflow-auto">
                {cart.map((item) => (
                  <div key={item.product_id} className="py-3 flex items-start justify-between gap-2">
                ...
                {cart.length === 0 && (
                  <p className="text-xs text-slate-500 py-4 text-center">Cart is empty</p>
                )}
              </div>
            </div>
